...
def new(request):
    if request.POST:
        ...
        if form.is_valid():
            if form.save():
                return redirect('/', messages.success(request, 'Successfully Reserved a Flight!', 'alert-success'))
            else:
...
def edit(request, order_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            if form.save():
                return redirect('/', messages.success(request, 'Booking was successfully Updated.', 'alert-success'))
            else:
                return redirect('/', messages.error(request, 'Data is not saved', 'alert-danger'))
        else:
            return redirect('/', messages.error(request, 'Form is not valid', 'alert-danger'))
...
def destroy(request, order_id):
    order = Order.objects.get(id=order_id)
    order.delete()
    return redirect('/', messages.success(request, 'Booking Cancelled', 'alert-success'))
...
def new_product(request):
    if request.POST:
        ...
        if form.is_valid():
            if form.save():
                return redirect('/products', messages.success(request, 'Flight created and available!', 'alert-success'))
            else:
...
def destroy_product(request, product_id):
    ...
    if Product.objects.filter(id=product_id).update(active='0'):
        return redirect('/products', messages.success(request, 'Flight successfully deleted!', 'alert-success'))  
    else:
        return redirect('/products', messages.danger(request, 'Cannot delete flight while its booking exists.', 'alert-danger'))  